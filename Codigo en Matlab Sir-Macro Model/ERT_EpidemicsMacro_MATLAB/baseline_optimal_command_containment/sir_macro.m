...
deltav=0;%1/52;        %Weekly probability of discovering a vaccine
deltac=0;%1/52;        %Weekly probability of discovering a treatment
kappa=0;%0.9;%0;            %Slope of pid-function in endog. pid scenario
...
if use_parallel==0
     opts_fmincon=optimoptions('fmincon','Display','iter','TolFun',1e-4,'MaxFunctionEvaluations',30000,'FiniteDifferenceStepSize',1e-1); %options for fmincon w/o parallel comp.
elseif use_parallel==1
      opts_fmincon=optimoptions('fmincon','Display','iter','MaxIterations',30000,'OptimalityTolerance',1e-7,'ConstraintTolerance',1e-7,'MaxFunctionEvaluations',1000000,'UseParallel',true,'FiniteDifferenceStepSize',1e-3,'Algorithm','interior-point');
end
...
n_guess=nrss*ones(HH,1);
c_guess=crss*ones(HH,1);

guess=[n_guess;c_guess];
...
LB=0*guess;
UB=0*guess+1e10;
[command_sol,fval,exitflag] = fmincon(@getU,guess,[],[],[],[],LB,UB,@nonlinconstraint,opts_fmincon,A,theta,i_ini,pop_ini,pis1,pis2,pis3,pir,pid,betta,Uiss,HH,crss,nrss,Urss,phii,deltav,deltac,kappa);

%save solution for possible use in subsequent maximization
save last_solution_command_containment command_sol;
...
[U0,c,ceq,I,S,R,D,T,Pop,cs,ns,Us,RnotSIRmacro,aggC,aggH,ci,cr,ni,nr,Ui,Ur,U,pid_endo]=getU(command_sol,A,theta,i_ini,pop_ini,pis1,pis2,pis3,pir,pid,betta,Uiss,HH,crss,nrss,Urss,phii,deltav,deltac,kappa);
RnotSIRmacro
U0
...
suptitle('Figure XX1: Command Containment in the Basic SIR-Macro Model');
orient landscape
print -dpdf -fillpage epidemic_simulation_figXX1
...
suptitle('Figure XX2: Command Containment in the Basic SIR-Macro Model');
orient landscape
print -dpdf -fillpage epidemic_simulation_figXX2


%Save workspace
save results_command_containment_basicSIRmacro
